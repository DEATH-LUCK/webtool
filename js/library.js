...
async function loadBooks() {
  try {
    ...
    allFolders = fSnap.docs.filter(d => d.data().scope !== 'library').map(d => ({ id: d.id, parent: d.data().parent || null }));
    const bSnap = await db.collection('books').orderBy('uploadedAt', 'desc').get();
    allBooks = bSnap.docs.filter(d => d.data().libraryScope !== 'library').map(d => ({ id: d.id, ...d.data() }));
    renderBooks();
  } catch(e) {
