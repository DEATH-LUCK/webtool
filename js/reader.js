// ============================================================
// READER.JS — Mobile-first PDF Reader with swipe support
// ============================================================
...
async function openReader(book) {
  ...
  const dlBtnMobile = document.getElementById('mobileDownloadBtn');
  if (dlBtn) dlBtn.href = book.downloadUrl;
  if (dlBtnMobile) dlBtnMobile.href = book.downloadUrl;
  ...
  try {
    ...
    epubEl.innerHTML = `
      <div style="width:100%; height:80vh; border-radius:8px; overflow:hidden;">
        <iframe src="${googleUrl}" style="width:100%; height:100%; border:none;" allowfullscreen></iframe>
      </div>`;
    document.getElementById('pageInfo').textContent = 'Viewer';
    document.getElementById('mobilePageInfo').textContent = 'Viewer';
  }
}
...
async function renderPage(pageNum) {
  ...
  try {
    ...
    const maxWidth = isMobile
      ...
      : Math.min(container.clientWidth - 48, 900);
    ...
    document.getElementById('pageInfo').textContent = `Page ${pageText}`;
...
async function openTXT(url) {
  try {
    ...
    function renderChunk() {
      epubEl.innerHTML = `<pre style="white-space:pre-wrap; font-family:var(--font-body); line-height:1.9; font-size:1rem; color:var(--text);">${chunks[currentPage-1]?.replace(/&/g,'&amp;').replace(/</g,'&lt;') || ''}</pre>`;
